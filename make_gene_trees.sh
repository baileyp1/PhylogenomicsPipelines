...
makeGeneTree()	{
    ...
    # $2 = filtered alignment filename in fasta format - NB - 24.8.2024 - in the case of a codon aln, the '/codonAln/' folder is included in $2 parameter 
	# $3 = output file prefix including the path to any directory (i.e. outputs generated by this method)
...
get_emma_stats() 	{
	###########
    # Function: gets stats for the raw the EMMA alignment (before any filtering)

    # Input parameter:
    residueType=$1 		# residue type (DNA, protein or codon)
    unalignedFile=$2 	# seqs before EMMA alignment 
    alignedFile=$3 		# seqs after EMMA alignment 
    ###########

    > ${gene}.${residueType}.emma_stats.txt	# Compiling stats in this file rather than the log file

	echo "##########################################################################" >> ${gene}.${residueType}.emma_stats.txt
   	echo "EMMA alignment stats for gene ${gene} (before any filtering and trimming!)" >> ${gene}.${residueType}.emma_stats.txt
   	echo "##########################################################################" >> ${gene}.${residueType}.emma_stats.txt
   	echo "Total number of residues in starting sequences (before EMMA): "`fastalength  $2 | awk '{sum+=$1} END {print sum}' ` >> ${gene}.${residueType}.emma_stats.txt
   	echo "Total number of residues in aligned sequences (after EMMA): "`fastalength  $3 | awk '{sum+=$1} END {print sum}' ` >> ${gene}.${residueType}.emma_stats.txt
   	echo "Number of gene sequences before EMMA alignment: "`cat $2 | grep '>' | wc -l ` >> ${gene}.${residueType}.emma_stats.txt
   	echo "Total number of samples in EMMA alignment: "`cat $3 | grep '>' | wc -l ` >> ${gene}.${residueType}.emma_stats.txt
   	echo "Number of unique samples in EMMA alignment: "`cat  $3 | grep '>' | sort -u | wc -l ` >> ${gene}.${residueType}.emma_stats.txt
   	echo "All the above numbers above should be identical." >> ${gene}.${residueType}.emma_stats.txt
   	echo "Number of EMMA sub-alignment fasta files: "`ls ${gene}.${residueType}.emma/sub-alignments/*.fasta | wc -l ` >> ${gene}.${residueType}.emma_stats.txt
   	echo "Total number of seqs in the sub-alignments: "`cat ${gene}.${residueType}.emma/sub-alignments/*.fasta | grep '>' | wc -l ` >> ${gene}.${residueType}.emma_stats.txt
   	echo "Number of unique samples in the sub-alignments: "`cat ${gene}.${residueType}.emma/sub-alignments/*.fasta | grep '>' | sort | uniq -c | wc -l ` >> ${gene}.${residueType}.emma_stats.txt
   	# Number of unique samples in the sub-alignments printed to file below:
   	cat ${gene}.${residueType}.emma/sub-alignments/*.fasta | grep '>' | sort | uniq -c | sed 's/>//' | awk '{print $2}' > ${gene}.${residueType}.emma/sub-alignments/${gene}_all_subsets_labels_sort_uniq-c.txt
   	echo "Number of samples in sub-alignments appearing more than once: "`cat ${gene}.${residueType}.emma/sub-alignments/*.fasta | grep '>' | sort | uniq -c | awk '$1 > 1' | wc -l ` >> ${gene}.${residueType}.emma_stats.txt
}
...
if [[ $geneFile != 'use_genewise_files' ]]; then
	...
	#      e.g.: using the file of all concatenated gene alignments together per sample (most extreme example)
	#      cat dna.alns.concatenated.fasta | seqtk seq -l 0 /dev/fd/0  | grep -v '>' | awk '{print $0}' | wc -L (-L finds the length of the longest line)
	#	   28.6.2024 - I think this was longer than than ARG_MAX or other max line variable limits yet all seqs were still on a single line - good - check again 
	...
	### a gene the pipeline will exit - this is probaably a good thing to happen, no?
	...
	### 8.7.2024 - still would be good to do this - # seqs > 3 is checked later - OK? and check - actually would be useful for smnall test  datasets
	echo "...done"
...
#	None of them existing triggers the error.
	...
	### 16.1.2024 - what about protein and codon files??? Also need to extend this list
		echo "WARNING: the input gene-wise fasta file (modified to <gene_name>_dna.fasta) for this gene does not exist or is empty: $gene - skipping alignment of this gene.
(It indicates that there are no samples for this gene (possibly after a filtering step), the gene list is not completely compatible with the input fasta files
or the input gene-wise fasta files were already called <gene_name>_dna.fasta and are now called <gene_name>_dna_dna.fasta which is not recognised!)"
