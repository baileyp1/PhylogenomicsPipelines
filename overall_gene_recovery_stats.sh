...
medianReadDepthWithDups_min4x numbrBasesInAllGenes_ReadDepth_min0x numbrBasesInAllGenes_ReadDepth_min4x numbrBasesInAllGenes_ReadDepthWithDups_min4x > readDepthPerGene_st_depth.txt
...
while read gene; do
	...
	depth=0
	# Number of recovered bases in ALL genes with read depth >= 0 (i.e. total of all bases):
	numbrBasesInAllGenes_ReadDepth_min0x=`cat Sample_*/*_bwa_mem_sort_st_depth.txt | awk -v depth=$depth '$3 >= depth' | wc -l `
    ...
    depth=4
     # Mean read depth concatfor bases with >= 4x depth  across ALL genes:
	meanReadDepth3=`cat Sample_*/*_bwa_mem_sort_st_depth.txt | awk -v depth=$depth '$3 >= depth' | awk '{sum+=$3} END {if(sum > 0) {print sum/NR} else {print "0"}}' `	# average
...
$medianValue4 $numbrBasesInAllGenes_ReadDepth_min0x $numbrBasesInAllGenes_ReadDepth_min4x $numbrBasesInAllGenes_ReadDepthWithDups_min4x    
