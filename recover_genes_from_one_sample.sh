...
#> paftol_da_add_samples_batch_upload.log 2>&1 &
	...
	if [[ $hybSeqProgram == 'hybpiper' ]];then 
		...
		if [[ -s ${sampleId}/genes_with_seqs.txt ]]; then
			...
			# For HybPiper > v2.2.0, the chimera check is no longer on by default so the "paralogs_no_chimeras" might not exist so check it does:
			if [[ -d paralogs_no_chimeras ]]; then
...
if [[ $stats != 'no' ]]; then
	...
	# NB - 30.8.2024 - now put first here so if read mapping steps fail, at least the basic steps will already have been printed to the file! 
	# Number of recovered genes:
	numbrRecoveredGenes=`cat $refFileName | grep '>' | wc -l `
	# Sum length of genes:
	sumLengthOfGenesWithNs=`fastalength $refFileName | awk '{sum+=$1} END {print sum}' `	# This calculation is OK for Paftools and HybPiper recovery methods so far; for HybPiper2 need to remove
	
	# Also removing strings of N's from the sequence line before counting the number of bases
	# NB - now using this code on all recovery methods; but it will also remove a (very) 
...
sumLengthOfGenes: $sumLengthOfGenes" > ${sampleId}_gene_recovery_stats${reexonrtOptionLog}.txt  # Also wipes out file contents from any previous run
	...
	if [[ $stats == 'yes:u' ]]; then
		# Output the unmapped reads if requested and convert to a fastq file.
